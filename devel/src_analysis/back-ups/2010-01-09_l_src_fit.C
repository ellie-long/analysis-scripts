...
/*	Double_t fitf(Double_t *x, Double_t *par)
	...
	func->SetParameters(500,histCoverHe->GetMean(),histCoverHe->GetRMS());
	func->SetParNames("Constant","Mean_Value","Sigma");
*/	TF1 *fit1 = new TF1("fit1","[0]",1.36,1.75);
	fit1->SetParNames("Fit Value");
	Double_t fitval = fit1->GetParameter(0);
	Double_t fiterror = fit1->GetParError(0);
	fit1->SetLineColor(kRed);
	histCoverHe->Fit("fit1","R");
	...
	TString imagename = "xbj_scaling_plots/Longitudinal_xbj_scaling_plots_at_Q2_of_";
	imagename += Q2;
	imagename += ".png";
	c1->Print(imagename);
