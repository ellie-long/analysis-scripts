...
//  int endHeRunNumber=20879;
  ...
  TString Q2 = "1.0";
  TString vartodraw = "PriKineL.Q2 / (PriKineL.nu * 2 * 0.9315)";
...
//	TCut eventtype = "D.evtypebits&(1<<1)>0";
	...
	TCut gold = "ExTgtCor_L.dp>-0.04 && ExTgtCor_L.dp<0.058";
	TCut target = "ReactPt_L.z>-0.155 && ReactPt_L.z<0.175";
...
// Below is the loop for adding chains for the Carbon run
  for (int thisCRunNumber=CRunNumber; thisCRunNumber<(endCRunNumber+1); thisCRunNumber++)
  {
		...
		if(thisCRunNumber==1829){thisCRunNumber=999999;}

   for (int t=0; t<1000;t++)
    {
      filenameC = "/home/ellie/physics/e05-102/terabyte/ROOTfiles/e05102_L_";
...
// Below is the loop for adding chains for the 3He run
  ...
  for (int thisHeRunNumber=HeRunNumber;thisHeRunNumber<(endHeRunNumber+1); thisHeRunNumber++)
  {
	...
	if(thisHeRunNumber==1699){thisHeRunNumber=1704;}
	...
	if(thisHeRunNumber==1830){thisHeRunNumber=1832;}
	if(thisHeRunNumber==1831){thisHeRunNumber=1832;}
	if(thisHeRunNumber==1884){thisHeRunNumber=1906;}
	if(thisHeRunNumber==1885){thisHeRunNumber=1906;}
...
//	thisHeRunNumber = 999999999;
	...
	chainC->Draw("ExTgtCor_L.dp>>CdpCut(400,-0.08,0.08)", gold, "");
	...
	chainC->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>CSieveNoCut(100,-0.05,0.05,100,-0.1,0.1)", target, "");
	...
	chainC->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>CSieveCut(100,-0.05,0.05,100,-0.1,0.1)", target && sieve, "");
...
//	titleC += target;
	...
	histC->Scale(1/0.0063175);
	histC->Draw();
	TF1 *fitC1 = new TF1("fitC1","[0]*exp(-x*[1])",1.40,1.6);
...
//	titleHe += target;
	...
	histHe->Scale(1/2.407866);
	histHe->Draw();
	TF1 *fitHe1 = new TF1("fitHe1","[0]*exp(-x*[1])",1.40,1.6);
