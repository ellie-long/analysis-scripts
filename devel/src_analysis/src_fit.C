...
// Below is for Q2=1, 3rd pass runs
/*
...
// Below is for Q2=0.1s, 1st pass runs
  int CRunNumberR=20530;
  int endCRunNumberR=20532;
  int CRunNumberL=1644;
  int endCRunNumberL=1646;
  int HeRunNumberR=20460;
  int endHeRunNumberR=20540;
  int HeRunNumberL=1589;
  int endHeRunNumberL=1654;
  TString Q2 = "0.13";
  double CchargeScaleR=1/0.0014306;
  double CchargeScaleL=1/0.0014304;
  double HechargeScaleR=1/0.472101;
  double HechargeScaleL=1/0.48419;
...
//	TCut eventtypeR = "D.evtypebits&(1<<1)>0";
	...
	TCut CtargetR = "(ReactPt_R.z>-0.132 && ReactPt_R.z<-0.122) || (ReactPt_R.z>-0.068 && ReactPt_R.z<-0.056) || (ReactPt_R.z>0.002 && ReactPt_R.z<0.01) || (ReactPt_R.z>0.068 && ReactPt_R.z<0.078) || (ReactPt_R.z>0.132 && ReactPt_R.z<0.142)";
	TCut sieveR = "abs(ExTgtCor_R.ph)<0.03 && abs(ExTgtCor_R.th)<0.06";

	TCut cutsR = kinematicsR && targetR && sieveR && goldR && eventtypeR;
	TCut CcutsR = kinematicsR && CtargetR && sieveR && goldR && eventtypeR;
...
//	TCut eventtypeL = "(D.evtypebits&2)==2";
	...
	TCut CtargetL = "(ReactPt_L.z>-0.132 && ReactPt_L.z<-0.122) || (ReactPt_L.z>-0.068 && ReactPt_L.z<-0.056) || (ReactPt_L.z>0.002 && ReactPt_L.z<0.01) || (ReactPt_L.z>0.068 && ReactPt_L.z<0.078) || (ReactPt_L.z>0.132 && ReactPt_L.z<0.142)";
	TCut sieveL = "abs(ExTgtCor_L.ph)<0.03 && abs(ExTgtCor_L.th)<0.06";

	TCut cutsL = kinematicsL && targetL && sieveL && goldL && eventtypeL;
	TCut CcutsL = kinematicsL && CtargetL && sieveL && goldL && eventtypeL;
...
// Below is the loop for adding chains for the Right 3He runs
  ...
  for (int thisHeRunNumberR=HeRunNumberR;thisHeRunNumberR<(endHeRunNumberR+1); thisHeRunNumberR++)
  {
	...
	// Below is for skipping 1st pass non-He runs
	if(thisHeRunNumberR==20512){thisHeRunNumberR=20533;}

	// Below is for skipping 3rd pass non-He runs
...
// Below is the loop for adding chains for the Left 3He runs
  ...
  for (int thisHeRunNumberL=HeRunNumberL;thisHeRunNumberL<(endHeRunNumberL+1); thisHeRunNumberL++)
  {
	...
	// Below is for skipping 1st pass non-He runs
	if(thisHeRunNumberL==1639){thisHeRunNumberL=1647;}
...
//	thisHeRunNumberL = 999999999;
	...
	chainCR->Draw("ReactPt_R.z>>CReactZCutR(400,-0.3,0.3)", CtargetR, "");
	chainCL->Draw("ReactPt_L.z>>CReactZNoCutL(400,-0.3,0.3)", "", "");
	chainCL->Draw("ReactPt_L.z>>CReactZCutL(400,-0.3,0.3)", CtargetL, "");
	...
	chainCR->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>CThetaPhiNoCutR(100,-0.05,0.05,100,-0.1,0.1)", CtargetR, "");
	chainCL->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>CThetaPhiNoCutL(100,-0.05,0.05,100,-0.1,0.1)", CtargetL, "");
	...
	chainCR->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>CThetaPhiCutR(100,-0.05,0.05,100,-0.1,0.1)", CtargetR && sieveR, "");
	chainCL->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>CThetaPhiCutL(100,-0.05,0.05,100,-0.1,0.1)", CtargetL && sieveL, "");
	...
	TH2F *Q2WHe = new TH2F("Q2WHe",titleQ2WHe,100,0,0.8,400,0,1.6);
	chainHeR->Draw("PriKineR.Q2:PriKineR.nu>>Q2WHeR(100,0,0.8,400,0,1.6)", cutsR, "");
	chainHeL->Draw("PriKineL.Q2:PriKineL.nu>>Q2WHeL(100,0,0.8,400,0,1.6)", cutsL, "");
	...
	TH2F *Q2WC = new TH2F("Q2WC",titleQ2WC,100,0,0.8,400,0,1.6);
	chainCR->Draw("PriKineR.Q2:PriKineR.nu>>Q2WCR(100,0,0.8,400,0,1.6,100)", CcutsR, "");
	chainCL->Draw("PriKineL.Q2:PriKineL.nu>>Q2WCL(100,0,0.8,400,0,1.6,100)", CcutsL, "");
...
//	titleC += target;
	...
	histCR->Sumw2();
	histCL->Sumw2();
	chainCR->Draw("PriKineR.Q2 / (PriKineR.nu * 2 * 0.9315)>>histCR", CcutsR, "E");
	chainCL->Draw("PriKineL.Q2 / (PriKineL.nu * 2 * 0.9315)>>histCL", CcutsL, "E");
	histCR->Scale(CchargeScaleR);
	histCL->Scale(CchargeScaleL);
...
//	titleHe += target;
	...
	histHeR->Scale(HechargeScaleR);
	histHeL->Scale(HechargeScaleL);
...
//	titleCoverHe += target;
	...
	TString imagename = "xbj_scaling_plots/Vertical_xbj_scaling_plots_at_Q2_of_";
	imagename += Q2;
	imagename += ".png";
	c1->Print(imagename);
