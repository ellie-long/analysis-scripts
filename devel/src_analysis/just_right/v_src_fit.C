...
void v_src_fit(){
  ...
  int endCRunNumber=22462;
  int HeRunNumber=20579;
//  int endHeRunNumber=20647;
  int endHeRunNumber=20643;
...
//	TCut eventtype = "D.evtypebits&(1<<1)>0";
	...
	TCut gold = "ExTgtCor_R.dp>-0.04 && R.gold.dp<0.058";
	TCut target = "ReactPt_R.z>-0.155 && ReactPt_R.z<0.175";
...
// Below is the loop for adding chains for the Carbon run
  for (int thisCRunNumber=CRunNumber; thisCRunNumber<(endCRunNumber+1); thisCRunNumber++)
  {
		...
		if(thisCRunNumber==20735){thisCRunNumber=22380;}
...
//	thisHeRunNumber = 999999999;
	...
	chainC->Draw("ExTgtCor_R.dp>>CdpCut(400,-0.08,0.08)", gold, "");
	...
	chainC->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>CSieveNoCut(100,-0.05,0.05,100,-0.1,0.1)", target, "");
	...
	chainC->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>CSieveCut(100,-0.05,0.05,100,-0.1,0.1)", target && sieve, "");
