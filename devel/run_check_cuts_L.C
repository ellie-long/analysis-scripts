...
void run_check_cuts_L(int RunNumber){
	...
	TCut cut = "";
	TCut kinematics = "PriKineL.Q2<10 && GoodElectron==1 && PriKineL.omega<10";
//	TCut eventtype = "(D.evtypebits&2)==2";
	...
	TCut target1 = "ReactPt_L.z>-0.3 && ReactPt_L.z<-0.175";
	TCut target2 = "ReactPt_L.z>0.19 && ReactPt_L.z<0.3";
//	TCut sieve = "abs(ExTgtCor_L.ph)<0.02 && abs(ExTgtCor_L.th)<0.04";
	TCut sieve = "";
...
*/
		...
		{
			filenameHe = "/home/ellie/physics/e05-102/terabyte/ROOTfiles/e05102_L_";
	...
	chainHe->Draw("ReactPt_L.z>>HeReactZNoCut(400,-0.3,0.3)", eventtype && tracks, "");
	HeReactZNoCut->SetTitle(titleTarget);
	HeReactZNoCut->Draw();
	chainHe->Draw("ReactPt_L.z>>HeReactZCut(400,-0.3,0.3)",target && eventtype && tracks,"same");
	HeReactZCut->SetFillColor(kViolet);
	HeReactZCut->Draw("same");

/*	chainHe->Draw("ReactPt_L.z>>HeReactZCut1(400,-0.3,0.3)",target1 && eventtype && tracks,"same");
	chainHe->Draw("ReactPt_L.z>>HeReactZCut2(400,-0.3,0.3)",target2 && eventtype && tracks,"same");
...
*/
	...
	chainHe->Draw("PriKineL.Q2>>histQ2(400,0,1.6)", eventtype && tracks, "");
...
//	histQ2->SetStats(kFALSE);
	...
	chainHe->Draw("PriKineL.nu>>histNu(100,0.0,0.8)", eventtype && tracks, "");
...
//	histNu->SetStats(kFALSE);
	...
	chainHe->Draw("ExTgtCor_L.dp>>HedpNoCut(400,-0.08,0.08)", gold && target && eventtype && tracks, "");
	...
	TH2F *HeScat = new TH2F("HeScat",titleScat,300,-0.02,0.02,300,10,20);
/*	chainHe->Draw("ScaAngle:ExTgtCor_L.ph>>HeScat1(300,-0.02,0.02,300,10,20)",gold && target1 && eventtype && tracks,"");
	chainHe->Draw("ScaAngle:ExTgtCor_L.ph>>HeScat2(300,-0.02,0.02,300,10,20)",gold && target2 && eventtype && tracks,"");
	HeScat->Add(HeScat1,HeScat2);
*/
	chainHe->Draw("ScaAngle:ExTgtCor_L.ph>>HeScat",gold && target && eventtype && tracks,"");
	HeScat->Draw("COLZ");

	pad6->cd();
//	TString titleThetaPhi = "Theta and Phi (No Cut) for Run # ";
	TString titleThetaPhi = "ExTgtCor_L.th and ExTgtCor_L.ph for Run # ";
...
/*	TH2F *HeSieveNoCut1 = new TH2F("HeSieveNoCut1",titleThetaPhi,100,-0.05,0.05,100,-0.1,0.1);
	...
	chainHe->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>HeSieveNoCut1", target1 && eventtype && tracks, "");
	chainHe->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>HeSieveNoCut2", target2 && eventtype && tracks, "");
	HeSieveNoCut->Add(HeSieveNoCut1,HeSieveNoCut2);
*/
	chainHe->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>HeSieveNoCut", target && eventtype && tracks, "");
//	chainHe->Draw("L.tr.d_th:L.tr.d_ph>>HeSieveNoCut", target && eventtype && tracks, "");
	HeSieveNoCut->SetStats(kFALSE);
	HeSieveNoCut->Draw("COLZ");

	pad7->cd();
//	TString titleThetaPhi = "Theta and Phi (Cut) for Run # ";
	TString titleThetaPhi = "ExTgtCor_L.th and ExTgtCor_R.ph for Run # ";
	titleThetaPhi += RunNumber;
	cout << "Drawing " << titleThetaPhi << "..."  << endl;
...
/*	TH2F *HeSieveCut1 = new TH2F("HeSieveCut1",titleThetaPhi,100,-0.05,0.05,100,-0.1,0.1);
	...
	chainHe->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>HeSieveCut1", target1 && sieve && eventtype && tracks, "");
	chainHe->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>HeSieveCut2", target2 && sieve && eventtype && tracks, "");
	HeSieveCut->Add(HeSieveCut1,HeSieveCut2);
*/
	chainHe->Draw("ExTgtCor_L.th:ExTgtCor_L.ph>>HeSieveCut", target && sieve && eventtype && tracks, "");
//	chainHe->Draw("L.tr.r_th:L.tr.r_ph>>HeSieveCut", target && sieve && eventtype && tracks, "");
	...
	histx2->Sumw2();
	histx->Sumw2();
/*	chainHe->Draw("(PriKineL.Q2 / (PriKineL.nu * 2 * 0.9315))>>histx1", target1 && eventtype && tracks, "E");
	chainHe->Draw("(PriKineL.Q2 / (PriKineL.nu * 2 * 0.9315))>>histx2", target2 && eventtype && tracks, "E");
	histx->Add(histx1,histx2);
*/	
	
	chainHe->Draw("(PriKineL.Q2 / (PriKineL.nu * 2 * 0.9315))>>histx", target, "E");
	histx->Draw();
