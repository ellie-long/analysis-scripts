...
const char* psfile = "long_Ay_nu_1";

using namespace std;

datafile_t datafiles[] = {
//{ "data file.dat", "Data Name", "Q2", "Ay", "Error Up", "Error Down", ?, ?, Shape, Color}
  { "asym_datafiles/Ay_Long_nu_q2_1.txt", "This Experiment", "[0]", "[1]", "sqrt([2]*[2]+[3]*[3])", "sqrt([2]*[2]+[3]*[3])", 0,0,20,6 },
  { NULL }
};  
...
//  { "asym_theory/mami_ay_theory_full.txt", "FSI and MEC (Faddeev)", "[0]", "[1]", "0", "0", 0, 0, 2, 3 },
  { "asym_theory/vert_1_qe_peak.txt", "Central QE Peak", "[0]", "[1]", "0", "0", 0, 0, 2, 3 },
  { NULL }
};
...
void long_Ay_nu_1() {
  ...
  frm->GetYaxis()->SetTitle("Ay for Q2=0.953 (GeV/c)2");
  frm->SetMinimum(-0.02);
//  frm->SetMinimum(0);
//  frm->SetMaximum(1.0);
  frm->SetMaximum(0.025);
  frm->UseCurrentStyle();
  frm->Draw();
  frm->SetAxisRange(0.320,0.680,"X");
