...
const char* psfile = "long_NMR_ed";

using namespace std;

datafile_t datafiles[] = {
//{ "data file.day", "Data Name", "Q2", "NMR_ed", "Error Up", "Error Down", ?, ?, Shape, Color}
//  { "nmr_datafiles/vert_pol.dat", "Vertical", "[0]", "[1]", "sqrt([2]*[2]+[3]*[3])", "sqrt([2]*[2]+[3]*[3])", 0,0,20,4 },
  { "nmr_datafiles/long_pol.dat", "Longitudinal", "[0]", "[1]", "sqrt([2]*[2]+[3]*[3])", "sqrt([2]*[2]+[3]*[3])", 0,0,20,1 },
  { "nmr_datafiles/trans_pol.dat", "Transverse", "[0]", "[1]", "sqrt([2]*[2]+[3]*[3])", "sqrt([2]*[2]+[3]*[3])", 0,0,20,2 },
  { NULL }
};  
...
void long_NMR_ed() {
  ...
  cn->Draw();
  cn->UseCurrentStyle();
  TH1F *frm = new TH1F("NMR_ed","",1454,2181,3635);
  frm->SetStats(kFALSE);
  frm->GetXaxis()->SetTitle("Run #");
...
//  TLegend *legDta = new TLegend(.7,.2,.875,.35,"","brNDC");
  TLegend *legDta = new TLegend(.2,.2,.375,.35,"","brNDC");
