...
const char* psfile = "tauf2f1_kees";
...
TGraphAsymmErrors *fromGEntotauF2F1(TGraph *ogr) {
	...
	for (int i=0; i<npts; i++) {
		...
		Gd = gdipole(&Q2,0);
		Gm = MU*Gd;
		tau = Q2/(4.0*0.939*0.939);
		oy = y;
		y = 1.0*(Gm-oy)*tau/(Gm*tau+oy);
		ey1 = ey1*fabs(Gm*(tau+1.0)*tau/pow( Gm*tau+oy,2.0));
		ey2 = ey2*fabs(Gm*(tau+1.0)*tau/pow( Gm*tau+oy,2.0));
		gr->SetPoint(i,x,y);
...
Double_t genff(Double_t *x, Double_t *p) {
	...
	return (gm-gm*Ge_Gm)*tau/(gm*tau+gm*Ge_Gm);
}
...
void tauf2f1_kees() {
	...
	TCanvas *cn = new TCanvas("cn", "tau F2^n/F1^n", 900, 600);
	cn->Draw();
	cn->UseCurrentStyle();
	...
	TLatex *note = new TLatex(2.1371, 0.64876, "Values are not corrected for Glauber");
	note->SetTextSize(0.0369);
	note->Draw("same");

#ifdef STAT_ONLY
	  TLatex *statonlynote = new TLatex(2.1371, 0.982063, "Errors are statistical only");
