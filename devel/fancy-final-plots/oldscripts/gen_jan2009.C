...
#ifdef SPLIT_DATLEG
  TLegend *legDta2 = new TLegend(.2520,.4004,.5882,.9089,"","brNDC");
  legDta2->SetBorderSize(0); // turn off border
  legDta2->SetFillStyle(0);
#endif
	  ...
	  if (gr) {
		  if (f->lnpt) {
			  ...
			  if( nplot % 2 == 0 ){
				  ...
				  legDta2->AddEntry(gr,f->label,f->lnpt);
			  }
#endif
		  }
		  gr->SetLineStyle(0);
...
#ifdef SHIFT_TITLE
  TPaveLabel *shifted = new TPaveLabel( 0.0127, 0.3, 0.1638, 0.5, "G_{En}", "NDC");
