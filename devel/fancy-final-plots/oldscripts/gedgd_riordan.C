...
// { "e02013_nofsi.dat", "x","[0]","[1]","[2]","[2]",0,0,22,2},
 ...
 {"gen_newprop.dat", "PR09-016 (Galster)","[0]","(1.91*[0]*(1.0+[0]/0.71)**(-2))/(4.0*0.938*0.938+5.6*[0])","sqrt([2]*[2]+[3]*[3])","sqrt([2]*[2]+[3]*[3])",0,0,20,2},
  { NULL }
};  
...
Double_t f1dugen0(Double_t *x, Double_t *p ){
	Double_t Q2=x[0];
	Double_t tau = Q2/(4.0*0.938*0.938);
	return (2.0*(tau*GMn(Q2)  ) + GEp(Q2) + tau*GMp(Q2))/
		(tau*GMn(Q2)  + 2.0*(tau*GMp(Q2) + GEp(Q2)) );
...
TGraphAsymmErrors *fromGEntransform(TGraph *ogr);
...
void gedgd_riordan() {
	...
	frm->GetYaxis()->SetTitle("G_{E}^{d}/G_{D}");
...
TGraphAsymmErrors *fromGEntransform(TGraph *ogr) {
	...
	Double_t den2, dFdGEn, dFdGMn, dFdGEp, dFdGMp, GD;

	for (int i=0; i<npts; i++) {
		Double_t ex1=0,ex2=0,ey1=0,ey2=0;
		ogr->GetPoint(i,x,y);
		Q2=x;
		Double_t tau=Q2/(4*.938*0.938);
		if (ogrE) {
		...
		ey1 = sqrt( pow(dFdGEn*ey1,2.0) + pow( dFdGMn*dGMn(Q2), 2.0 ) +
				pow(dFdGEp*dGEp(Q2),2.0) + pow( dFdGMp*dGMp(Q2),2.0));
		ey2 = ey1;
		gr->SetPoint(i,x,y);
