...
void gen_kees() {
  ...
  TCanvas *cn = new TCanvas("cn", "GEn", 900, 600);
  cn->Draw();
  cn->UseCurrentStyle();
...
//  legThry->AddEntry(lomon, "Lomon", "l");
 ...
 // legThry->AddEntry(ourfit, "Our Fit", "l");
  ...
  TLatex *note = new TLatex(2.4028, 0.03617, "Values are not corrected for Glauber");
  note->SetTextSize(0.0369);
  note->Draw("same");

#ifdef STAT_ONLY
  TLatex *statonlynote = new TLatex(2.4028, 0.04325, "Errors are statistical only");
