...
const char* psfile = "long_n2pressure";

using namespace std;

datafile_t datafiles[] = {
//{ "data file.dat", "Data Name", "Q2", "n2pressure", "Error Up", "Error Down", ?, ?, Shape, Color}
  { "pressure_curve/n2-q2-05.txt", "N2 Pressurve Curve", "[0]", "[1]", "sqrt([2]*[2]+[3]*[3])", "sqrt([2]*[2]+[3]*[3])", 0,0,26,9 },
  { NULL }
};  
...
void long_n2pressure() {
  gROOT->SetStyle("HALLA");
  TCanvas *cn = new TCanvas("cn","cn",540,360);
  cn->Draw();
  cn->UseCurrentStyle();
  TH1F *frm = new TH1F("frm","",100,0.,40.);
  frm->GetXaxis()->SetTitle("Absolute Pressurve (psi)");
  frm->GetYaxis()->SetTitle("Nitrogen Yield");
  frm->SetMinimum(0);
//  frm->SetMinimum(0);
  frm->SetMaximum(80000000);
//  frm->SetMaximum(0.04);
  frm->UseCurrentStyle();
  frm->Draw();
  frm->SetAxisRange(0.,40,"X");
