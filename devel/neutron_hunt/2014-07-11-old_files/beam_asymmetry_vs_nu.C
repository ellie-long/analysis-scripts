...
// ********* Transverse, Q2=1.0 *******************
	int HeRunNumber = 22448;
	int endHeRunNumber = 22448;
	TString Q2 = "1.0";
	double q2min = 0.75;
	double q2max = 1.2;
	double numin = 0.3;
...
*/			handpad03->Update();
			...
			if (includeVetos) {chainHe->Draw(HANDSpinUpxString, cuts && vetoTDCs && goodTDCcut && firstTrack && "g0hel.R.helicity==1","");}
			if (!includeVetos) {chainHe->Draw(HANDSpinUpxString, cuts && "g0hel.R.helicity==1","");}
			...
			if (includeVetos) {chainHe->Draw(HANDSpinDownxString, cuts && vetoTDCs && goodTDCcut && firstTrack && "g0hel.R.helicity==-1","");}
			if (!includeVetos) {chainHe->Draw(HANDSpinDownxString, cuts && "g0hel.R.helicity==-1","");}
