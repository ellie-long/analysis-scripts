...
void hand_beam_asymmetry_vs_nu_background(){
...
//	int whichbar=11;
	...
	TH1F *TDCupTotal = new TH1F(plotTDCupTotal,titleTDCupTotal,TDCbins*2,TDCmin,TDCmax*2);
	...
	TH1F *TDCdownTotal = new TH1F(plotTDCdownTotal,titleTDCdownTotal,TDCbins*2,TDCmin,TDCmax*2);
	...
	for (int nplane=whichplane; nplane<endplane; nplane++)
	{
		...
		for (int thisbar=whichbar; thisbar<maxbars; thisbar++)
		{
			...
			handpad04 =  new TPad("handpad04","handpad04",0.1000,0.2500,0.5500,0.5000,0,0,0);
			handpad05 =  new TPad("handpad05","handpad05",0.5500,0.2500,1.0000,0.5000,0,0,0);
			handpad06 =  new TPad("handpad06","handpad06",0.1000,0.0000,0.5500,0.2500,0,0,0);
			handpad07 =  new TPad("handpad07","handpad07",0.5500,0.0000,1.0000,0.2500,0,0,0);
			handpad08 =  new TPad("handpad08","handpad08",0.1000,0.5000,0.3250,0.7500,0,0,0);
			handpad09 =  new TPad("handpad09","handpad09",0.3250,0.5000,0.5500,0.7500,0,0,0);
			handpad10 =  new TPad("handpad10","handpad10",0.5500,0.5000,0.7750,0.7500,0,0,0);
			handpad11 =  new TPad("handpad11","handpad11",0.7750,0.5000,1.0000,0.7500,0,0,0);
			handpad01->Draw(); handpad02->Draw(); handpad03->Draw(); handpad04->Draw(); handpad05->Draw();
			handpad06->Draw(); handpad07->Draw(); handpad08->Draw(); handpad09->Draw(); handpad10->Draw(); handpad11->Draw();
...
//			HANDrightcut->Scale(1/(HeChargeScale*Helivetime));
			...
			handpad08->cd();
			TString titleTDCup = "(TDCl + TDCr) Spin Up for Plane #";
			titleTDCup += nplane;
			titleTDCup += ", PMT # ";
			titleTDCup += thisbar;
			cout << "Drawing " << titleTDCup << "..." << endl;
			TString plotTDCup = "TDCup_p";
			plotTDCup += nplane;
			plotTDCup += "_b";
			plotTDCup += thisbar;
			TString TDCupevents = "(";
			TDCupevents += goodTDCl;
			TDCupevents += " + ";
			TDCupevents += goodTDCr;
			TDCupevents += ")>>";
			TDCupevents += plotTDCup;
			TH1F *TDCup = new TH1F(plotTDCup,titleTDCup,TDCbins*2,TDCmin,TDCmax*2);
			chainHe->Draw(TDCupevents, cuts && vetoTDCs && firstTrack && goodTDCcut &&"((g0hel.R.helicity==1 && g.runnum<22467) || (g0hel.R.helicity==-1 && g.runnum>22468))","");
			TDCup->SetMinimum(0);
			TDCup->Draw();
			TDCupFactors = new TPaveText(0.11,0.69,0.4,0.89,"NDC");
			TString TDCupFactorsString = "Total # of channels: ";
			TDCupFactorsString += (TDCmax-TDCmin);
			TString TDCupFactorsString1 = "# Channels Cut: ";
			TDCupFactorsString1 += (goodTDCleftcutmax - goodTDCleftcutmin);
			TString TDCupFactorsString2 = "# Channels Used: ";
			TDCupFactorsString2 += ((TDCmax-TDCmin) - (goodTDCleftcutmax - goodTDCleftcutmin));
			TText *t1 = TDCupFactors->AddText(TDCupFactorsString);
			TText *t2 = TDCupFactors->AddText(TDCupFactorsString1);
			TText *t3 = TDCupFactors->AddText(TDCupFactorsString2);
			...
			TH1F *TDCdown = new TH1F(plotTDCdown,titleTDCdown,TDCbins*2,TDCmin,TDCmax*2);
