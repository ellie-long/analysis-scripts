...
#include <math.h>
#include <iomanip>
#include "hand_define_vetos_for_bar_by_thph.h"
#include "hand_draw.h"
...
//	TString outputPlotsHere = "/home/ellie/physics/e05-102/images/plots_for_hand/left_right_position_test/";
//	TString outputPlotsHere = "/home/ellie/physics/e05-102/images/plots_for_hand/testing-something-left_right_position_test/firstTrack_withcuts_";
	TString outputPlotsHere = "/home/ellie/physics/e05-102/images/plots_for_hand/testing-delete-me/";
...
//	int whichbar=11;
	for (int nplane=whichplane; nplane<endplane; nplane++)
	{
		...
		for (int thisbar=whichbar; thisbar<maxbars; thisbar++)
		{
			...
			handpad02 =  new TPad("handpad02","handpad02",0.1000,0.7500,0.5500,1.0000,0,0,0);
			handpad03 =  new TPad("handpad03","handpad03",0.5500,0.7500,1.0000,1.0000,0,0,0);
			handpad04 =  new TPad("handpad04","handpad04",0.5500,0.0000,1.0000,0.5000,0,0,0);
			handpad05 =  new TPad("handpad05","handpad05",0.1000,0.0000,0.5500,0.5000,0,0,0);
			handpad06 =  new TPad("handpad06","handpad06",0.1000,0.5000,0.5500,0.7500,0,0,0);
			handpad07 =  new TPad("handpad07","handpad07",0.5500,0.5000,1.0000,0.7500,0,0,0);
...
//			HANDrightcut->Scale(1/(HeChargeScale*Helivetime));
			...
			handpad03->Update();
                        ...
                        handpad06->cd();
                        handpad06->SetLogy();
                        cout << "Drawing Left ADC..." << endl;
                        TString titleHANDleftadc = "Left ADC for Plane #";
                        titleHANDleftadc += nplane;
                        titleHANDleftadc += ", PMT # ";
                        titleHANDleftadc += thisbar;
                        TString titleHANDleftadccut = titleHANDleftadc;
                        titleHANDleftadccut += " with goodTDC cuts";
                        if (includeVetos) {titleHANDleftadc += " with vetos";}
                        else {titleHANDleftadc += " without vetos";}
                        cout << "Drawing " << titleHANDleftadc << "..." << endl;
                        TString plotHANDleftadc = "HANDleftadc_p";
                        plotHANDleftadc += nplane;
                        plotHANDleftadc += "_b";
                        plotHANDleftadc += thisbar;
                        TString plotHANDleftadccut = plotHANDleftadc;
                        plotHANDleftadccut += "_cut";
                        TH1F *HANDleftadc = new TH1F(plotHANDleftadc,titleHANDleftadc,ADCbins,ADCmin,ADCmax);
                        TH1F *HANDleftadccut = new TH1F(plotHANDleftadccut,titleHANDleftadccut,ADCbins,ADCmin,ADCmax);
                        TString HANDleftADC = goodADCl;
                        HANDleftADC += ">>";
                        HANDleftADC += plotHANDleftadc;
                        TString HANDleftADCcut = goodADCl;
                        HANDleftADCcut += ">>";
                        HANDleftADCcut += plotHANDleftadccut;
                        chainHe->Draw(HANDleftADC, cuts,"");
                        HANDleftadc->SetMinimum(1);
                        HANDleftadc->Draw();
                        chainHe->Draw(HANDleftADCcut, cuts && goodTDCcut && firstTrack,"same");
                        HANDleftadccut->SetFillColor(kViolet);
                        HANDleftadccut->SetMinimum(1);
                        HANDleftadccut->Draw("same");
//                      TF1 *leftFit = new TF1("leftFit","gaus",ADCmin,ADCmax);
//                      HANDleftadc->Fit(leftFit, "R");
//                      leftFit->Draw("same");
                        handpad06->Update();

                        handpad07->cd();
                        handpad07->SetLogy();
                        cout << "Drawing Right ADC..." << endl;
                        TString titleHANDrightadc = "Right ADC for Plane #";
                        titleHANDrightadc += nplane;
                        titleHANDrightadc += ", PMT # ";
                        titleHANDrightadc += thisbar;
                        TString titleHANDrightadccut = titleHANDrightadc;
                        titleHANDrightadccut += " with goodTDC cuts";
                        if (includeVetos) {titleHANDrightadc += " with vetos";}
                        else {titleHANDrightadc += " without vetos";}
                        cout << "Drawing " << titleHANDrightadc << "..." << endl;
                        TString plotHANDrightadc = "HANDrightadc_p";
                        plotHANDrightadc += nplane;
                        plotHANDrightadc += "_b";
                        plotHANDrightadc += thisbar;
                        TString plotHANDrightadccut = plotHANDrightadc;
                        plotHANDrightadccut += "_cut";
                        TH1F *HANDrightadc = new TH1F(plotHANDrightadc,titleHANDrightadc,ADCbins,ADCmin,ADCmax);
                        TH1F *HANDrightadccut = new TH1F(plotHANDrightadccut,titleHANDrightadccut,ADCbins,ADCmin,ADCmax);
                        TString HANDrightADC = goodADCr;
                        HANDrightADC += ">>";
                        HANDrightADC += plotHANDrightadc;
                        TString HANDrightADCcut = goodADCr;
                        HANDrightADCcut += ">>";
                        HANDrightADCcut += plotHANDrightadccut;
                        chainHe->Draw(HANDrightADC, cuts,"");
                        HANDrightadc->SetMinimum(1);
                        HANDrightadc->Draw();
                        chainHe->Draw(HANDrightADCcut, cuts && goodTDCcut && firstTrack,"same");
                        HANDrightadccut->SetFillColor(kViolet);
                        HANDrightadccut->SetMinimum(1);
                        HANDrightadccut->Draw("same");
			handpad07->Update();
...
//			TH2F *HeThetaPhiNoCut = new TH2F("HeThetaPhiNoCut",titleThetaPhiNoCut,100,-0.05,0.05,100,-0.025,0.025);
			chainHe->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>HeThetaPhiNoCut", cuts, "");
			...
			handpad04->cd();
			TString thiscutstring = "NA.n.plane=";
			thiscutstring += nplane;
			thiscutstring += " && NA.n.pad=";
			thiscutstring += thisbar;
			TCut thiscut = thiscutstring;
			cout << "Drawing Theta and Phi kinematics..." << endl;
			TString kinematicstitleThetaPhikinematics = "Theta and Phi with Purple Cuts (from above)";
			TH2F *HeThetaPhikinematics = new TH2F("HeThetaPhikinematics",kinematicstitleThetaPhikinematics,100,-0.05,0.05,100,-0.1,0.1);
//			TH2F *HeThetaPhikinematics = new TH2F("HeThetaPhikinematics",kinematicstitleThetaPhikinematics,100,-0.05,0.05,100,-0.025,0.025);
			chainHe->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>HeThetaPhikinematics", cuts && goodTDCcut && vetoTDCs && firstTrack, "");
	//		chainHe->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>HeThetaPhikinematics", cuts && goodTDCcut && vetoTDCs, "");
//			chainHe->Draw("ExTgtCor_R.th:ExTgtCor_R.ph>>HeThetaPhikinematics", cuts && thiscut && vetoTDCs, "");
			HeThetaPhikinematics->SetTitle(kinematicstitleThetaPhikinematics);
//			HeThetaPhikinematics->SetStats(kFALSE);
			HeThetaPhikinematics->Draw("COLZ");
...
//			TH2F *HeThetaPhiNokinematics = new TH2F("HeThetaPhiNokinematics",kinematicstitleThetaPhiNokinematics,100,-0.05,0.05,100,-0.1,0.1);
			TH2F *HeThetaPhiNokinematics = new TH2F("HeThetaPhiNokinematics",kinematicstitleThetaPhiNokinematics,100,-0.05,0.05,100,-0.025,0.025);
...
//			psshcut->Draw("same");
			...
			if (thisbar < 10) {imageKinematicsTitle += "0";}
