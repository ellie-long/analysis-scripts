...
//
//	2011-03-15
//		Changed to just give the beam asymmetry
//		without looking for neutrons. This is
//		to make it easier to see if my asymmetries
//		match those of JinGe
// ***************************************************
...
//	const int xBjbins = 10;
	...
	TString outputPlotsHere = "/home/ellie/physics/e05-102/images/plots_for_hand/test-hand_beam_asymmetry/";
...
//	int whichbar=11;
	...
	for (int nplane=whichplane; nplane<endplane; nplane++)
	{
		...
		for (int thisbar=whichbar; thisbar<maxbars; thisbar++)
		{
			...
			TString theCurrentBarString = "NA.n.plane==";
			theCurrentBarString += nplane-1;
			if (nplane==0) {theCurrentBarString = "";}
			theCurrentBarString += " && NA.n.pad==";
			theCurrentBarString += thisbar;
			TCut theCurrentBar = theCurrentBarString;
			cout << endl << "theCurrentBarString: " << theCurrentBarString << endl << endl;;
...
//			HANDleft->Scale(1/(HeChargeScale*Helivetime));
			HANDleft->Draw();
			if (!includeVetos) {chainHe->Draw(HANDleftTDCcut, cuts && theCurrentBar && firstTrack,"same");}
			if (includeVetos) {chainHe->Draw(HANDleftTDCcut, cuts && vetoTDCs && goodTDCleftcut && theCurrentBar && firstTrack,"same");}
...
//			HANDright->Scale(1/(HeChargeScale*Helivetime));
			HANDright->Draw();
			if (!includeVetos) {chainHe->Draw(HANDrightTDCcut, cuts && theCurrentBar && firstTrack,"same");}
			if (includeVetos) {chainHe->Draw(HANDrightTDCcut, cuts && vetoTDCs && goodTDCrightcut && theCurrentBar && firstTrack,"same");}
...
*/			handpad03->Update();
			...
			if (includeVetos) {chainHe->Draw(HANDSpinUpxString, cuts && vetoTDCs && goodTDCcut && theCurrentBar && firstTrack && "((g0hel.R.helicity==1 && g.runnum<22467) || (g0hel.R.helicity==-1 && g.runnum>22468))","");}
//			if (!includeVetos) {chainHe->Draw(HANDSpinUpxString, cuts && theCurrentBar && firstTrack && "g0hel.R.helicity==1","");}
			if (!includeVetos) {chainHe->Draw(HANDSpinUpxString, cuts && "((g0hel.R.helicity==1 && g.runnum<22467) || (g0hel.R.helicity==-1 && g.runnum>22468))","");}
			...
			if (includeVetos) {chainHe->Draw(HANDSpinDownxString, cuts && vetoTDCs && goodTDCcut && theCurrentBar && firstTrack && "((g0hel.R.helicity==-1 && g.runnum<22467) || (g0hel.R.helicity==1 && g.runnum>22468))","");}
//			if (!includeVetos) {chainHe->Draw(HANDSpinDownxString, cuts && theCurrentBar && firstTrack && "g0hel.R.helicity==-1","");}
			if (!includeVetos) {chainHe->Draw(HANDSpinDownxString, cuts && "((g0hel.R.helicity==-1 && g.runnum<22467) || (g0hel.R.helicity==1 && g.runnum>22468))",""); cout << "Peek-a-boo!" << endl;}
