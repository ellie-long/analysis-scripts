...
void test_hand_beam_asymmetry_vs_nu(){
...
//	const int nuBins = 50;
	...
	TString outputPlotsHere = "/home/ellie/physics/e05-102/images/plots_for_hand/test_hand_beam_asymmetry_vs_nu/";
...
//			HANDleft->Scale(1/(HeChargeScale*Helivetime));
			HANDleft->Draw();
			if (!includeVetos) {chainHe->Draw(HANDleftTDCcut, cuts && vetoTDCs && firstTrack,"same");}
...
//			HANDright->Scale(1/(HeChargeScale*Helivetime));
			HANDright->Draw();
			if (!includeVetos) {chainHe->Draw(HANDrightTDCcut, cuts && firstTrack,"same");}
