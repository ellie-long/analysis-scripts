// ***************************************************
//
// This script is use to see how the ADC spectrum
// looks in comparison to the TDC spectrum. It is going
// to be used mostly for calibrating the *.dat files
// for the neutron detector's ADC spectrums
//
// 2011-01-31
// Elena Long
//
...
*/
/*
	int HeRunNumber = 22493;
	int endHeRunNumber = 22493;
	TString Q2 = "1.0";
	double q2min = 0.75;
	double q2max = 1.2;
	double numin = 0.3;
...
*/
	...
	TString outputPlotsHere = "/home/ellie/physics/e05-102/images/plots_for_hand/adc_calibration_test/";
...
// This bit of code should set the charge and livetime scaling factors
	double HeChargeScale = 0;
	double Helivetime = 0;
	hand_scaling_factors(HeRunNumber, endHeRunNumber, HeChargeScale, Helivetime);
...
//	int whichplane = 1;
	...
	for (int nplane=whichplane; nplane<endplane; nplane++)
	{
		...
		for (int thisbar=whichbar; thisbar<maxbars; thisbar++)
		{
			...
			handpad04 =  new TPad("handpad04","handpad04",0.1000,0.0000,0.5500,0.5000,0,0,0);
			handpad05 =  new TPad("handpad05","handpad05",0.5500,0.0000,1.0000,0.5000,0,0,0);
...
//			HANDrightcut->Scale(1/(HeChargeScale*Helivetime));
			...
			chainHe->Draw(HANDleftADC, "" ,"");
			HANDleftadc->SetMinimum(1);
			HANDleftadc->Draw();
			chainHe->Draw(HANDleftADCcut, cuts && goodTDCcut,"same");
...
//			leftFit->Draw("same");
			...
			chainHe->Draw(HANDrightADC, "","");
			HANDrightadc->SetMinimum(1);
			HANDrightadc->Draw();
			chainHe->Draw(HANDrightADCcut, cuts && goodTDCcut,"same");
...
/*
			...
			HANDrightadc->Draw();
//			TF1 *rightFit = new TF1("rightFit","gaus",ADCmin,ADCmax);
//			HANDrightadc->Fit(rightFit, "R");
//			rightFit->Draw("same");
*/			handpad05->Update();


			TString imageTitle = outputPlotsHere;
			imageTitle += "ADC_plots_Run_Num_";
