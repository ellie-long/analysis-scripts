...
//	histPosHelx->Add(histPosHelxIn,histPosHelxOut);
	chainHe->Draw("(PriKineR.Q2 / (PriKineR.omega * 2 ))>>histPosHelx", cuts && "he3R.Spin==1", "");
	// NOTE: Line above, 1.202242 = Coulombs in the positive state
	histPosHelx.Scale(1/1.202242);
	histPosHelx->Draw();
...
//	histNegHelx->Add(histNegHelxIn,histNegHelxOut);
	chainHe->Draw("(PriKineR.Q2 / (PriKineR.omega * 2 * 0.9315))>>histNegHelx", cuts && "he3R.Spin==-1", "");
	// NOTE: Line above, 1.167498 = Coulombs in the negative state	
	histNegHelx.Scale(1/1.167498);
	histNegHelx->Draw();
