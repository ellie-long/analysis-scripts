...
//			Auto Runs
//		Feb 2009    Jin Huang
//			Avoid searching for extra segments of raw data file, if replay event number reachs request
//			Add Header File to support compiling
//
//////////////////////////////////////////////////////////////////////////
				...
				)
{
	...
	for (Int_t nsplit=0;!exit;nsplit++)
	{
		...
		if ( ((!path || !*path) && !found) || oldfilename==filename) {
			...
			if (NeveAna>=(UInt_t)nev) exit=true;
			run->Close();
