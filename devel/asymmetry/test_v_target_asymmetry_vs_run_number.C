...
void test_v_target_asymmetry_vs_run_number(){

	ifstream fruns ("/home/ellie/physics/e05-102/analysis-scripts/devel/info_from_internet/run_number");
	string frunline;
	vector<string> runNum;
	while( getline(fruns,frunline)) runNum.push_back(frunline);
	cout << "# of Lines in Run Number: " << runNum.size() << "\n";
	fruns.close();

	ifstream fu3pos1 ("/home/ellie/physics/e05-102/analysis-scripts/devel/info_from_internet/BCM/bcmu3++");
	string fu3pos1line;
	vector<string> u3pos1;
	while( getline(	fu3pos1,fu3pos1line)) u3pos1.push_back(fu3pos1line);
	cout << "# of Lines in u3 [++]: " << u3pos1.size() << "\n";
	fu3pos1.close();

	ifstream fu3pos2 ("/home/ellie/physics/e05-102/analysis-scripts/devel/info_from_internet/BCM/bcmu3+-");
	string fu3pos2line;
	vector<string> u3pos2;
	while( getline(	fu3pos2,fu3pos2line)) u3pos2.push_back(fu3pos2line);
	cout << "# of Lines in u3 [+-]: " << u3pos2.size() << "\n";
	fu3pos2.close();

	ifstream fu3neg1 ("/home/ellie/physics/e05-102/analysis-scripts/devel/info_from_internet/BCM/bcmu3-+");
	string fu3neg1line;
	vector<string> u3neg1;
	while( getline(	fu3neg1,fu3neg1line)) u3neg1.push_back(fu3neg1line);
	cout << "# of Lines in u3 [-+]: " << u3neg1.size() << "\n";
	fu3neg1.close();

	ifstream fu3neg2 ("/home/ellie/physics/e05-102/analysis-scripts/devel/info_from_internet/BCM/bcmu3--");
	string fu3neg2line;
	vector<string> u3neg2;
	while( getline(	fu3neg2,fu3neg2line)) u3neg2.push_back(fu3neg2line);
	cout << "# of Lines in u3 [--]: " << u3neg2.size() << "\n";
	fu3neg2.close();


// Below is a test to make sure that the vectors have gotten the correct values into them. 
// It's working as of 2009-02-02.	
/*	for (int i=0; i < (runNum.size()); i++)
	{
		cout << runNum[i] + "  " + u3pos1[i] + "  " + u3pos2[i] + "  " + u3neg1[i] + "  " + u3neg2[i] << endl;
	}
*/
...
//		Note: Uncomment the line below unles you only want to go through one run. This is good for testing purposes.
		thisHeRunNumber = 99999999999;
	}
	...
	chainHe->Draw("he3R.Spin:(PriKineR.Q2 / (PriKineR.omega * 2 * 0.9315))>>histHel", cuts, "");
	histHel->Draw("COLZ");
