void qe_cross_section()
{
	...
	double alpha = 1/137.035999679;	
//	double mott = ((4*(7.29735257e-3)*(7.29735257e-3)*3.8937e-32)*Eprime*Eprime*(cos(theta/2)*cos(theta/2))*(1/(q2*q2)))*(Eprime/E);
	double mott = (alpha*alpha/(4*E*E))*((cos(theta/2)*cos(theta/2))/(pow(sin(theta/2),4)))*(Eprime/E);

	cout << "(dsigma/dOmega)_Mott = " << mott << " m^2" << endl;
//	cout << "(dsigma/dOmega)_Mott = " << mott*pow(10,-37) << " nb" << endl;
	cout << "-------------------------------" << endl;
...
//	double neutroncs = ((4*(7.29735257e-3)*(7.29735257e-3)*3.8937e-32)*Eprime*Eprime*(cos(theta/2)*cos(theta/2))*(1/(q2*q2)))*(Eprime/E)*(((GEn*GEn + tau*GMn*GMn)/(1+tau)) + 2*tau*GMn*GMn*(tan(theta/2)*tan(theta/2)));
	...
	double protoncs = mott*(((GEp*GEp + tau*GMp*GMp)/(1+tau)) + 2*tau*GMp*GMp*(tan(theta/2)*tan(theta/2)));
	double neutroncs = mott*(Eprime/E)*(((GEn*GEn + tau*GMn*GMn)/(1+tau)) + 2*tau*GMn*GMn*(tan(theta/2)*tan(theta/2)));
	cout << "Assuming free nucleons, we find:" << endl;
	cout << "(dsigma/dOmega)_p = " << protoncs << " m^2" << endl;
//	cout << "(dsigma/dOmega)_p = " << protoncs*pow(10,-37) << " nb" << endl;
	cout << "(dsigma/dOmega)_n = " << neutroncs << " m^2" << endl;
//	cout << "(dsigma/dOmega)_n = " << neutroncs*pow(10,-37) << " nb" << endl;
	cout << " - - - - - - - - - - - - - - - " << endl;
